const BookingApp = {
    ...
    setupAutocomplete(fieldId) {
        ...
        input.addEventListener('input', () => {
            ...
            debounceTimer = setTimeout(async () => {
                try {
                    const response = await fetch('https://api.pegasustransport.se/api/Map/AutoComplete', {
                        method: 'POST',
    ...
    async getCoordinates(sessionToken, placeId, fieldId) {
        try {
            ...
            const response = await fetch(`https://api.pegasustransport.se/api/Map/GetLongNLat?${params}`);
            const result = await response.json();
            if (result.data) {
